...
class LoanService:
    ...
    @staticmethod
    def calcular_deducciones_prestamos(empleado, salario_bruto):
        """
        Calcula las deducciones totales por préstamos para un empleado.
        Maneja múltiples préstamos activos, deduciendo del más antiguo primero.
        Limita las deducciones al 50% del salario bruto después de deducciones legales.

        Args:
            empleado: Instancia del modelo Empleado
            salario_bruto (float): Salario bruto calculado

        Returns:
            dict: {'total_deducciones': float, 'detalles': list of dicts}
        """
        ...
        total_deducciones = 0
        detalles = []
        limite_deducciones = salario_bruto * 0.5  # 50% del salario bruto

        for prestamo in prestamos_activos:
            if total_deducciones >= limite_deducciones:
                break
            ...
            deduccion_actual = min(detalle_pago['pago_mensual'], limite_deducciones - total_deducciones)
            total_deducciones += deduccion_actual

            # Actualizar saldo del préstamo
            prestamo.saldo_restante = detalle_pago['nuevo_saldo']
            ...
            detalles.append({
                ...
                'deduccion': deduccion_actual,
                'saldo_restante': prestamo.saldo_restante
            })
